...
**Everything below gives the game away.** The puzzles, the secrets, what the
ring really does, how to get out of the Elvenking's halls, where the keys are
and what the numbers underneath it all look like.

The game is designed to be found out by playing it. Gandalf tells you what he
scouts, locked doors name the key you're missing, and `help` covers the verbs.
You do not need this page to finish it.

If you want it anyway -- because you're stuck, because you're curious how it
works, or because you'd rather read than play -- read on.
...
## The dark places, and what they take

- **In the goblin tunnels a companion can be dragged off.** One at Bilbo's side
  is far harder to snatch, but -- as in the tale, where the tunnels take the
  whole company -- not untouchable. Keeping together is protection, not
  immunity. Once the **Great Goblin is slain** the tunnels have no master and
  the abductions stop, so a rescue stays rescued.
...
- **Mirkwood has its own captors.** In the black of the forest **you cannot
  ...
  no use for gold and leave a prisoner his purse. The webbing stops once the
  great spider falls.
- **A capture can be undone.** Fight your way to the cell where a companion is
  held and you free them on the spot -- they rejoin the company and the rescue
  becomes part of its story. Gandalf can *find* the taken: a friend spotted in
  chains is urgent news, reported first, wherever he sees them. He also reads
  drag-marks and signs of a skirmish, so an abduction you never witnessed still
  reaches you.
...
- **`barrel` is a direction**, listed in the cellars' exits beside `up`. It tips
  the company into the empty barrels and out under the gate on the black water,
  shooting the rapids with elvish horns crying behind you. Barred ways show as
  `east (barred)` so a shut gate reads like a locked door.
...
The old map is a real object in the enhanced game. `examine map` in the right
place -- by moonlight -- reveals what the small silver key is for: the **Secret
Door** high on the Lonely Mountain, above a narrow ledge. Carry the key the
length of the world and `open door` names it as it turns.
...
A hard-pressed leader can **call for help**, which focuses the company's blows
on his attacker until it falls, and dwarves within earshot rush to a rally.

## Weapons, in numbers

Walking stick 4, Sting 7, Orcrist and Glamdring 8; Bilbo's bare hands are 3.

The **walking stick is a walking aid** that *lowers* your march fatigue (and
still beats bare fists in a pinch), while a **drawn sword is wearying** to
march with. Stroll with the stick; draw steel when Gandalf warns of trouble.

Hand a fallen friend's blade to another dwarf (`give orcrist to dwalin`) and
he'll wield it in the next fight.

## Wounds and healing

- A **badly wounded** fighter strikes weaker, as does a weak, hungry or
  exhausted one -- see it on `status` and in `party`.
...
## Food, weight and the pack

- Food is carried as **real items**, stacked -- e.g. `loaf of bread (x4)`. A
  pack holds a **weight**, not a count, shared by everything you carry (12 for
  Bilbo; the dwarves bear more, Bombur most). **Bread is light** -- Bilbo can
  shoulder a dozen loaves, Bombur two dozen -- so provisioning generously is
  easy; it's gold and steel that fill a pack.
...
- Everyone sets out with a single loaf. At a **food source** (the Green Dragon
  Inn, Rivendell, Beorn's Hall, Lake-town) the loaves never run out --
  `take loaf` grabs one, `stock up` fills your pack.
- **Each haven keeps its own fare.** Rivendell presses **waybread** on you (far
  more sustaining than a plain loaf, and just as light), Lake-town has cram, the
  Shire its loaves. Food found in the wild (mutton, waybread, cram) is picked up
  like any other item.
- Companions look after themselves: they eat when hungry, rest when spent,
  restock at settlements, and -- when their pack runs empty -- break off to
  forage at the nearest food source, then rejoin.
- **Collapse is survivable.** Weak from hunger you can no longer fight or march,
  but you can still **eat, rest and wait**, so there is always a way back if you
  have food. (In purist mode the original's death spiral stands: it tells you to
  eat, then refuses the command, and collapse is the end.)
...
- **`party` shows who bears what** -- treasure, keys, torches and weapons -- so
  you always know where the Arkenstone is, who has the key, and who's carrying
  the only torch before you head underground.
- **Weight decides who bears what.** The heap of treasure is simply too heavy
  for a hobbit; it takes a dwarf.
...
When Smaug falls the game does not stop there. The hoard lies open, and the
company **loads what it can bear** -- setting down the last of its bread to
make room, because the road is walked and nobody needs a week's rations walking
out of a hall they've just won.

What comes out is bounded by who survived to carry it. A strong company gets
the heap of treasure out; a company down to two hurt dwarves leaves it in the
Mountain, and the game names what was left behind.

If **Thorin lived** to see it, the Arkenstone is his and he says so -- the last
thing he speaks in the game. If he fell on the road, Bilbo pockets it in
silence, as he does in the book.

Then the roster shows who ended up bearing what, the reckoning totals it, and
only then are you told you have won. Treasure that fell with the dead, or that
